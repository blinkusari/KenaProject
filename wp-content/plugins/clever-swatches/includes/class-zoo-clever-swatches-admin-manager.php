...
if (!class_exists('Zoo_Clever_Swatch_Admin_Manager')) {
    class Zoo_Clever_Swatch_Admin_Manager
    {
        ...
        public function zoo_cw_edit_attr_fields($term)
        {
            ...
            $color_code = get_term_meta($id, 'slctd_clr', true);
            $image = get_term_meta($id, 'slctd_img', true);
            if (empty($image)) {
        ...
        public function zoo_cw_save_attr_extra_fields($term_id, $tt_id = '', $taxonomy = '')
        {
            ...
            if (isset($mylink)) {
                ...
                if ($display_type == 'color') {
                    update_term_meta($term_id, 'slctd_clr', $_POST['zoo_cw_slctdclr']);
                } elseif ($display_type == 'image') {
                    update_term_meta($term_id, 'slctd_img', $_POST['zoo-cw-selected-attr-img']);
                }
            }
        }
        ...
        public function zoo_cw_preview_column_content($columns, $column, $id)
        {
            if ('thumb' == $column) {
                ...
                if ($display_type == 'color') {
                    ...
                    $img_url = get_term_meta($id, 'slctd_img', true);
                    if ($img_url) {
                        $image = $img_url;
                    } else {
                        $image = wc_placeholder_img_src();
