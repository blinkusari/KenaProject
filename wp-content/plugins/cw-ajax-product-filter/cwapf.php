<?php
/**
 * Plugin Name: Clever WooCommerce Ajax Product Filters
 * Description: A lighweght plugin to filter woocommerce products with AJAX request.
 * Plugin URI: https://cleveraddon.com
 * Version: 1.0.0
 * Author: David
 * Text Domain: cwapf
...
if (!class_exists('CWAPF')) {
	class CWAPF
	{
		...
		{
			$this->define('CWAPF_LOCALE', $this->plugin_slug);
			$this->define('CWAPF_PATH', $this->pluginPath());
			$this->define('CWAPF_ASSETS_PATH', $this->assetsPath());
			$this->define('CWAPF_CACHE_TIME', 60*60*12);
		}
		...
		public function frontendScripts()
		{
			...
			wp_register_style('cwapf-style', CWAPF_ASSETS_PATH . 'css/cwapf-styles.css');

			wp_register_script('cwapf-script', CWAPF_ASSETS_PATH . 'js/scripts.js', array('jquery'), '20120206', true);
			wp_localize_script('cwapf-script', 'cwapf_price_filter_params', array(
				'currency_symbol' => get_woocommerce_currency_symbol(),
				'currency_pos'    => get_option('woocommerce_currency_pos')
			));

			if ($settings) {
				wp_localize_script('cwapf-script', 'cwapf_params', $settings);
			}

			wp_register_style('cwapf-nouislider-style', CWAPF_ASSETS_PATH . 'css/nouislider.min.css');
			wp_register_script('cwapf-nouislider-script', CWAPF_ASSETS_PATH . 'js/nouislider.min.js', array ('jquery'), '1.0', true);
			wp_register_script('cwapf-price-filter-script', CWAPF_ASSETS_PATH . 'js/price-filter.js', array ('jquery'), '1.0', true);

			wp_register_script('cwapf-select2', CWAPF_ASSETS_PATH . 'js/select2.min.js', array ('jquery'), '1.0', true);
		}
		...
		{
			add_options_page(__('WC Ajax Product Filter', 'cwapf'), __('WC Ajax Product Filter', 'cwapf'), 'manage_options', 'cwapf-settings', array($this, 'settingsPage'));
		}
		...
		public function defaultSettings()
		{
			return array(
				...
				'shop_breadcrumb_container'  => '.woocommerce-breadcrumb',
				'shop_result_count_container'  => '.woocommerce-result-count',
				'not_found_container'  => '.products',
		...
		{
			register_setting('cwapf_settings', 'cwapf_settings', array($this, 'validateSettings'));
		}
		...
		{
			if (!get_option('cwapf_settings')) {
				// check if filter is applied
				$settings = apply_filters('cwapf_settings', $this->defaultSettings());
				update_option('cwapf_settings', $settings);
			}
		}
		...
		{
			if (has_filter('cwapf_settings')) {
				$input = apply_filters('cwapf_settings', $input);
			}

			if (isset($input['clear_transients']) && $input['clear_transients'] == '1' ) {
				// clear transient
				cwapf_clear_transients();
			}
			...
			foreach ($query as $key => $value) {
				...
				if (preg_match('/^attr/', $key) && !empty($query[$key])) {
					...
					foreach ($terms as $term_id) {
						$ancestors = cwapf_get_term_ancestors($term_id, $taxonomy);
						$term_data = cwapf_get_term_data($term_id, $taxonomy);
				...
				if (preg_match('/product-cat/', $key) && !empty($query[$key])) {
					...
					if (preg_match('/^product-cat-and/', $key)) {
						$query_type = 'and';
					} else {
						$query_type = 'or';
					}
		...
		public function queryForMeta()
		{
			...
			$meta_query = apply_filters('cwapf_get_meta_query', $meta_query);
		...
		public function setFilter($q)
		{
			...
			$q->set('date_query', apply_filters('cwapf_get_date_query',array()));
			$q->set('post__in', apply_filters('cwapf_get_post__in',$post__in));
		...
		public function unfilteredProductIds()
		{
 			if (!is_tax(get_object_taxonomies('product'))) {
 				...
 				$transient_name = 'cwapf_unfiltered_product_ids';

 				if (false === ($unfiltered_product_ids = get_transient($transient_name))) {
 					$unfiltered_product_ids = get_posts($args);
 					set_transient($transient_name, $unfiltered_product_ids, cwapf_transient_lifespan());
 				}
		...
		public function unfilteredProductsPriceRange()
		{
			...
			$transient_name = 'cwapf_unfiltered_product_price_range';
			...
			if (false === ($unfiltered_products_price_range = get_transient($transient_name))) {
				...
				set_transient($transient_name, $unfiltered_products_price_range, cwapf_transient_lifespan());
			}
		...
		{
			echo '<div class="cwapf-before-products">';
		}
		...
		{
			load_plugin_textdomain('cwapf', FALSE, basename(dirname(__FILE__)) . '/languages/');
		}
		...
		public function needWoocommerce()
		{
			...
			echo '<p>' . __('WC Ajax Product Filter needs WooCommerce plugin to work.', 'cwapf') . '</p>';
			echo '</div>';
		}
		...
		{
			echo '<div class="error">';
			echo '<p>' . __('To use WC Ajax Product Filter update your WooCommerce plugin.', 'cwapf') . '</p>';
			echo '</div>';
		}
		...
		{
			$links[] = '<a href="' . admin_url('options-general.php?page=cwapf-settings') . '">' . __('Settings', 'cwapf') . '</a>';
			return $links;
		}
	}
}
