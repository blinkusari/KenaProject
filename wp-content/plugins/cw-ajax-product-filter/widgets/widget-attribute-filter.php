 ...
 */
if (!class_exists('CWAPF_Attribute_Filter_Widget')) {
	class CWAPF_Attribute_Filter_Widget extends WP_Widget {
		/**
		 * Register widget with WordPress.
		 */
		function __construct() {
			parent::__construct(
				'cwapf-attribute-filter', // Base ID
				__('CW Ajax Product Filter by Attribute', 'cwapf'), // Name
				array('description' => __('Filter woocommerce products by attribute.', 'cwapf')) // Args
			);
		}
		...
		public function widget($args, $instance) {
			...
			if ($display_type === 'dropdown') {
				$output = cwapf_dropdown_terms($attr_args);
			} else {
				$output = cwapf_list_terms($attr_args);
			}
		...
		public function form($instance) {
			...
			<p>
				...
				<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo (!empty($instance['title']) ? esc_attr($instance['title']) : __( 'Filter by', 'cwapf' )); ?>">
			</p>
			<p>
			<?php
			...
			if (sizeof($attribute_taxonomies) > 0) {
				?>
				<label for="<?php echo $this->get_field_id('attr_name'); ?>"><?php printf(__('Attribute', 'cwapf')); ?></label>
				...
				printf(__('No attribute found!', 'cwapf'));
			}
			?>
			</p>
			<p>
				...
				<select class="widefat" id="<?php echo $this->get_field_id('display_type'); ?>" name="<?php echo $this->get_field_name('display_type'); ?>">
					...
					<option value="button" <?php echo ((!empty($instance['display_type']) && $instance['display_type'] === 'button') ? 'selected="selected"' : ''); ?>><?php printf(__('Button', 'cwapf')); ?></option>
					<option value="swatch" <?php echo ((!empty($instance['display_type']) && $instance['display_type'] === 'swatch') ? 'selected="selected"' : ''); ?>><?php printf(__('Swatch', 'cwapf')); ?></option>
					<option value="dropdown" <?php echo ((!empty($instance['display_type']) && $instance['display_type'] === 'dropdown') ? 'selected="selected"' : ''); ?>><?php printf(__('Dropdown', 'cwapf')); ?></option>
				</select>
			</p>
			<p>
				...
				<select class="widefat" id="<?php echo $this->get_field_id('query_type'); ?>" name="<?php echo $this->get_field_name('query_type'); ?>">
					<option value="and" <?php echo ((!empty($instance['query_type']) && $instance['query_type'] === 'and') ? 'selected="selected"' : ''); ?>><?php printf(__('AND', 'cwapf')); ?></option>
					<option value="or" <?php echo ((!empty($instance['query_type']) && $instance['query_type'] === 'or') ? 'selected="selected"' : ''); ?>><?php printf(__('OR', 'cwapf')); ?></option>
				</select>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('enable_multiple'); ?>"><?php printf(__('Enable multiple filter', 'cwapf')); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_count'); ?>"><?php printf(__('Show count', 'cwapf')); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('hierarchical'); ?>"><?php printf(__('Show hierarchy', 'cwapf')); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_children_only'); ?>"><?php printf(__('Only show children of the current attribute', 'cwapf')); ?></label>
			</p>
			<?php
		}
...
// register widget
if (!function_exists('cwapf_register_attribute_filter_widget')) {
	function cwapf_register_attribute_filter_widget() {
		register_widget('CWAPF_Attribute_Filter_Widget');
	}
	add_action('widgets_init', 'cwapf_register_attribute_filter_widget');
}