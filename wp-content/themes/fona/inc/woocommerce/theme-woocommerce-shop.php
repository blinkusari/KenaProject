...
function zoo_button_wishlist() {
	if ( get_theme_mod( 'zoo_enable_wishlist', '1' ) == 1 ) {
		...
		echo apply_filters( 'zoo_button_wishlist', sprintf(
			...
			get_theme_mod( 'zoo_text_add_to_wishlist', esc_attr__( 'Add to wishlist', 'fona' ) ),
			esc_attr__( 'Product Added', 'fona' ),
			$icon,
			get_theme_mod( 'zoo_text_add_to_wishlist', esc_html__( 'Add to wishlist', 'fona' ) )
		) );
	} else {
		return false;
	}
...
function zoo_button_products_compare() {
	if ( get_theme_mod( 'zoo_enable_compare', '1' ) == 1 ) {
		...
		echo apply_filters( 'zoo_button_products_compare', sprintf(
			...
			$class,
			esc_attr( $product->get_id() ),
			get_theme_mod( 'zoo_text_add_to_compare', esc_attr__( 'Add to compare', 'fona' ) ),
			esc_attr__( 'Product Added', 'fona' ),
			$icon,
			get_theme_mod( 'zoo_text_add_to_compare', esc_html__( 'Add to compare', 'fona' ) )
		) );
	} else {
		return false;
	}
...
if ( ! function_exists( 'zoo_change_breadcrumb_delimiter' ) ) {
	function zoo_change_breadcrumb_delimiter( $defaults ) {
		...
		$defaults['delimiter'] = ' <span class="dotted"></span> ';

		return $defaults;
	}
}

if (get_theme_mod('zoo_disable_breadcrumbs', '0') == '0') {
    add_action('woocommerce_before_main_content', 'zoo_open_wrap_woo_breadcrumb', 15);
    add_action('woocommerce_before_main_content', 'zoo_close_wrap_woo_breadcrumb', 25);
...
function woocommerce_output_content_wrapper() {
	$allow_html = array( 'main' => array( 'id' => array(), 'class' => array() ) );
	echo wp_kses( '<main id="site-main-content" class="main-content">', $allow_html );
...
/**
 * Zoo shop page heading.
 * Display shop page heading, product category and taxonomy description.
 * With shop page, It will display content of shop page follow content field of that page.
 * @uses hook zoo_shop_heading to hook  woocommerce_archive_description
 * @return string html Shop page heading
 */
if ( ! function_exists( 'zoo_shop_heading' ) ) {
	function zoo_shop_heading() {
		get_template_part( 'inc/templates/woocommerce/shop', 'heading' );
	}
}
add_action( 'woocommerce_archive_description', 'zoo_shop_heading', 10 );
...
/**
 * Add wrapper for woocommerce_before_shop_loop and top products page
 * @close at zoo_after_shop_loop.
 * @uses Use function zoo_wrap_before_shop_loop and hook to woocommerce_before_shop_loop
 * @return custom html wrap open before shop loop of theme and wrap top products page.
 */
if ( ! function_exists( 'zoo_wrap_before_shop_loop' ) ) {
	function zoo_wrap_before_shop_loop() {
		$allow_html = array(
			'div' => array( 'class' => array(), 'id' => array() ),
		);
		$zoo_class  = 'zoo-wrap-product-shop-loop ' . zoo_wrap_shop_loop_class();
		echo wp_kses( '<div  class="' . $zoo_class . '">', $allow_html );
	}
}
add_action( 'woocommerce_before_main_content', 'zoo_wrap_before_shop_loop', 35 );
...
/**
 * Close Top block woocommerce_open_shop_loop
 * @uses hook this function to woocommerce_before_shop_loop
 * @return html wrap top products page.
 */
if ( ! function_exists( 'zoo_close_top_products_page' ) ) {
	function zoo_close_top_products_page() {
		$allow_html = array(
...
add_action( 'woocommerce_before_shop_loop', 'zoo_close_top_products_page', 40 );
	...
	function zoo_before_shop_loop() {
		if ( ! is_single() ):
			...
			if ( zoo_product_sidebar() == 'off-canvas' || zoo_product_sidebar() == 'top' || ! is_active_sidebar( $widget ) ) {
				echo wp_kses( '<div class="zoo-products-shop-loop">', $allow_html );
			} else {
...
if ( ! function_exists( 'zoo_after_shop_loop' ) ) {
	function zoo_after_shop_loop() {
		...
		if ( zoo_product_sidebar() == 'off-canvas' || zoo_product_sidebar() == 'top' ) {
			...
			echo wp_kses( '</div></div></div>', $allow_html );
		}
	}
}
...
if ( ! function_exists( 'zoo_template_quick_view_btn' ) ) {
	function zoo_template_quick_view_btn() {
		$quick_view_btn = '<a href="#" class="button btn-quick-view" title="' . esc_attr__( 'Quick view', 'fona' ) . '" data-productID="' . get_the_ID() . '"><i class="zoo-icon-eye"></i> ' . esc_html__( 'Quick view', 'fona' ) . '</a>';
...
if ( ! function_exists( 'zoo_shop_stock_label' ) ) {
	function zoo_shop_stock_label() {
		...
		if ( ! $product->is_in_stock() ) {
			?>
            <span class="out-stock-label zoo-stock-label"><?php esc_html_e( 'Out of Stock', 'fona' ); ?></span>
			<?php
		} else {
			if ( get_option( 'woocommerce_notify_low_stock_amount' ) > $product->get_stock_quantity() && $product->get_stock_quantity() ) {
				?>
                <span class="low-stock-label zoo-stock-label"><?php esc_html_e( 'Low Stock', 'fona' ); ?></span>
				<?php
			}
		}
	}
...
if ( ! function_exists( 'zoo_shop_new_label' ) ) {
	function zoo_shop_new_label() {
		...
		if ( get_post_meta( $product->get_id(), 'zoo_single_product_new', true ) == 1 ) {
			?>
            <span class="zoo-new-label"><?php esc_html_e( 'New', 'fona' ); ?></span>
			<?php
		}
	}
}
...
if ( get_theme_mod( 'zoo_shop_cart_icon', 'zoo-icon-cart' ) != '' ) {
	function zoo_get_icon_cart( $url, $product, $args ) {
		...
		if ($temp_icon_cart&& $temp_icon_cart['icon'] != '' ) {
...
function zoo_close_wrap_no_product() {
	...
	echo wp_kses( '<a href="' . get_permalink( wc_get_page_id( 'shop' ) ) . '" title="' . esc_attr__( 'Return to Shop', 'fona' ) . '" class="button return-shop">' . esc_html__( 'Return to Shop', 'fona' ) . '</a>', $allow_html );
	echo wp_kses( '</div>', $allow_html );
}
...
if ( ! function_exists( 'zoo_shop_pagination' ) ) {
	function zoo_shop_pagination( $args = array() ) {
		$args['prev_text'] = '<i class="zoo-icon-long-arrow-left"></i>';
		$args['next_text'] = '<i class="zoo-icon-long-arrow-right"></i>';
