 ...
 * Ajax Url
 */
add_action('wp_enqueue_scripts', 'zoo_checkout_url_render', 1000);
// Enqueue scripts for theme.
if (!function_exists('zoo_checkout_url_render')) {
    function zoo_checkout_url_render()
    {
        // Load custom style
        wp_add_inline_script('zoo-scripts', zoo_set_checkout_url());
    }
}
function zoo_set_checkout_url(){
    global $woocommerce;
    $checkout_page_url = function_exists( 'wc_get_cart_url' ) ? wc_get_checkout_url() : $woocommerce->cart->get_checkout_url();
    return 'const wc_checkout_url="'.$checkout_page_url.'"';
}