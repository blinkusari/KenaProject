...
$enable_shop_banner = get_theme_mod('zoo_enable_banner', 1);
if ($enable_shop_banner) {
    ...
    if ($thumb_img != '') {
        ?>
        <img src="<?php echo esc_url($thumb_img) ?>" alt="<?php echo woocommerce_page_title(); ?>"
             class="shop-heading-image"/>
        <?php
    }
    if ($enable_shop_des) {
        if (is_shop()) {
