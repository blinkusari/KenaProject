...
function zoo_theme_pre_activation_check($old_theme_name, WP_Theme $old_theme_object)
{
    try {
        ...
        $die_msg = sprintf('<h1 class="align-center">'.esc_html__('Theme Activation Error', 'fona').'</h1><p class="zoo-active-theme-error" >%s</p><p class="align-center"><a href="%s">'.esc_html__('Return to dashboard', 'fona').'</a></p>', $e->getMessage(), get_admin_url(null, 'index.php'));
        switch_theme($old_theme_object->stylesheet);
        wp_die($die_msg, esc_html__('Theme Activation Error', 'fona'), 500);
    }
...
function zoo_theme_default_setup()
{
    ...
    register_nav_menus([
        'top-menu'     => esc_html__('Top Menu', 'fona'),
        'mobile-menu'  => esc_html__('Mobile Menu', 'fona'),
        'primary-menu' => esc_html__('Primary Menu', 'fona'),
    ]);
 ...
 */
function zoo_register_elementor_locations($elementor_theme_manager) {
	$elementor_theme_manager->register_all_core_location(); // Full support.
}
add_action('elementor/theme/register_locations', 'zoo_register_elementor_locations');
