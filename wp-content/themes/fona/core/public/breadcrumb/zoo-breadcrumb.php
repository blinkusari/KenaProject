...
final class Zoo_Breadcrumb
{
    ...
    private function theYearCrumb(\WP_Query $query)
    {
        ...
        return $this->getTypeCrumb($query) . esc_html__('Posted in', 'fona') . ': ' . $y;
    }
    ...
    private function theMonthCrumb(\WP_Query $query)
    {
        ...
        return $this->getCrumbLink(get_year_link($y), $y) . $this->getSep() . esc_html__('Posted on', 'fona') . ': ' . $m;
    }
    ...
    {
        return esc_html__('Posted by', 'fona') . ': ' . $query->queried_object->display_name;
    }
    ...
    {
        return esc_html__('Search results for', 'fona') . ': ' . htmlspecialchars($query->query_vars['s'], ENT_QUOTES);
    }
    ...
    {
        return esc_html__('404 Not Found', 'fona');
    }
