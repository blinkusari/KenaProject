...
final class Zoo_Setup_Wizard
{
	...
	{
		$this->theme = wp_get_theme(get_option('template', 'fona'));
	...
	function _wizard_steps()
	{
		$steps = array(
			...
			'system_status' => array(
				'name'    => esc_html__( 'Preinstallation', 'fona' ),
				'view'    => array( $this, 'system_status' ),
				'handler' => '',
			),
			'child_theme' => array(
				'name'    => esc_html__( 'Child Theme', 'fona' ),
				'view'    => array( $this, 'child_theme' ),
				'handler' => '',
			),
			'default_plugins' => array(
				'name'    => esc_html__( 'Install Plugins', 'fona' ),
				'view'    => array( $this, 'default_plugins' ),
				'handler' => '',
			),
			'done_setup' => array(
				'name'    => esc_html__( 'Done', 'fona' ),
 ...
 */\n";
	...
	{
		?><p><?php esc_html_e('Before installing the theme, we need to check your system environment. The theme will not work properly if your system does not meet the requirements.', 'fona') ?></p>
        ...
        <tr>
            ...
            <span class="field-tooltip">
                <span class="dashicons dashicons-info"></span>
                <span class="tip-content">
                    <?php printf( esc_html__('%s requires PHP version 5.6 at least.', 'fona'), $this->theme->name ); ?>
				...
				<tr>
					...
					<td>
                        <span class="field-tooltip">
                            <span class="dashicons dashicons-info"></span>
                            <span class="tip-content">
                                <?php printf( esc_html__('%s requires WordPress version 4.6 at least.', 'fona'), $this->theme->name ); ?>
                                <?php esc_html_e('You should use latest version for better perfomance and security.', 'fona'); ?>
                            </span>
                        </span>
					</td>
		...
		<?php if (!$this->system_status) : ?>
			<p class="danger-text"><strong><?php esc_html_e('Whoops, your system does not meet the requirements. You should contact your hosting or VPS provider to fix it before installing this theme.', 'fona') ?></strong></p>
		<?php endif; ?>
		<p class="envato-setup-actions step">
			<?php
	...
	protected function child_theme()
	{
		...
		<p id="envato-setup-child-theme-text" class="<?php echo esc_attr($class_attr) ?>"><?php
			if ($is_child_theme) {
				printf( esc_html__('%sGreat! A child theme named as %s has already been created and activated successfully%s.', 'fona'), '<strong>', $child_theme_name, '</strong>' );
			} else {
				printf( esc_html__('If you are going to make changes, it&#8217;s better to use a %schild theme%s rather than modifying parent theme directly. By using a child theme, parent theme can be updated without overwriting your modifications. To generate a child theme, just click on the "Create child theme" button. After generated, feel free to make any change with that child theme which stored in the newly created child theme directory!', 'fona'), '<a href="https://codex.wordpress.org/Child_Themes" target="_blank">', '</a>' );
			}
		?></p>
		<form action="" method="post">
		 	<div class="child-theme-input">
		 		<label class="screen-reader-text" for="child_theme_name"><strong><?php esc_html_e('Child Theme Name', 'fona') ?></strong></label>
	...
	protected function done_setup()
	{
		...
		<form action="" method="post"><?php
			...
			elseif (true !== $this->tgmpa->is_tgmpa_complete()) :
				?><p><?php echo sprintf(esc_html__( 'Look like required plugins are not installed. Sample data cannot be installed without required plugins! Go to %sinstall plugins%s to install required plugins.', 'fona' ), '<a href="'.esc_url($this->get_step_link('default_plugins')).'">', '</a>'); ?></p><?php
                return;
            else:
				?><p class="lead success"><strong><?php esc_html_e( 'Cool! Theme and Plugins have been setup successfully!', 'fona' ) ?></strong></p><p><?php printf(esc_html__( 'It&#8217;s time to import demo data for your new website to make it look even better. %sNOTE THAT BY INSTALLING DEMO DATA, YOUR CURRENT DATA MAY BE OVERRIDDEN OR DUPLICATED%s. We recommend to import demo data on fresh sites or use %sWP Reset plugin%s to clean up your database before importing demo data.', 'fona' ), '<strong style="color:#ff5042">', '</strong>', '<a href="https://wordpress.org/plugins/wp-reset/" target="_blank">', '</a>'); ?></p><?php
			endif;
			?>
			<br>
			<p class="envato-setup-actions step">
				<a href="<?php menu_page_url('zoo-theme-setup-demo'); ?>" class="button-primary button button-large"><?php esc_html_e( 'Setup Demo Data Now', 'fona' ); ?></a>
				<a href="<?php echo esc_url( admin_url() ); ?>" class="button button-large"><?php esc_html_e( 'Not right now', 'fona' ); ?></a>
			</p>
		</form><?php
	}
	...
	{
		?><h1 id="zoo-heading-finish"><?php esc_html_e( 'Congratulations! Your website is ready!', 'fona' ); ?></h1>
		...
		<div class="envato-setup-next-steps">
			<div class="envato-setup-next-steps-first">
				<h2 ><?php esc_html_e( 'Next Steps', 'fona' ); ?></h2>
				<ul>
					...
					<li class="setup-product"><a class="button button-large" href="<?php echo esc_url(admin_url('customize.php')); ?>"><?php esc_html_e( 'Customize your website', 'fona' ); ?></a></li>
				</ul>
			</div>
			<div class="envato-setup-next-steps-last">
				<h2><?php esc_html_e( 'More Steps', 'fona' ); ?></h2>
				<ul>
					<li class="howto">
	...
	protected function get_base_content()
	{
		...
		if ( file_exists($base_dir . 'content.xml') ) {
            $content['content'] = array(
				'title'            => esc_html__('Content', 'fona'),
				'description'      => esc_html__('Sample content such as posts, pages, menus...', 'fona'),
		...
		if ( file_exists($base_dir . 'slider.zip') ) {
			$content['sliders'] = array(
				...
				'description'      => esc_html__('Sample sliders.', 'fona'),
				'pending'          => esc_html__('Pending', 'fona'),
				'installing'       => esc_html__('Installing', 'fona'),
				'success'          => esc_html__('Success', 'fona'),
		...
		if ( file_exists($base_dir . 'customizer.dat') ) {
			$content['options'] = array(
				'title'            => esc_html__('Thememods', 'fona'),
				'description'      => esc_html__('Sample theme customize data.', 'fona'),
				'pending'          => esc_html__('Pending', 'fona'),
				'installing'       => esc_html__('Installing', 'fona'),
				'success'          => esc_html__('Success', 'fona'),
        ...
        if ( file_exists($base_dir . 'widgets.wie') ) {
            $content['widgets'] = array(
                ...
                'description'      => esc_html__('Sample widgets.', 'fona'),
                'pending'          => esc_html__('Pending', 'fona'),
                'installing'       => esc_html__('Installing', 'fona'),
                'success'          => esc_html__('Success', 'fona'),
		...
		if ( file_exists($base_dir . 'settings') ) {
			$content['settings'] = array(
				...
				'description'      => esc_html__('Sample theme and plugins&#8217; settings.', 'fona'),
				'pending'          => esc_html__('Pending', 'fona'),
				'installing'       => esc_html__('Installing', 'fona'),
				'success'          => esc_html__('Success', 'fona'),
