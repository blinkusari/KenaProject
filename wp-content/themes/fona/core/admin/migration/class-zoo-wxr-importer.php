...
final class Zoo_WXR_Importer
{
    ...
    {
        if (!is_readable($file)) {
            throw new Exception(esc_html__('Importing file does not exist or is not readable.', 'fona'));
        }
        ...
        if (is_wp_error($data)) {
            ...
            throw new Exception(sprintf(esc_html__('Failed to parse import file. %s', 'fona'), $error_msg));
        }
        ...
        if ($is_image) {
            if (!$is_logo) {
                ...
                if (!empty($max_size) && $filesize > $max_size) {
                    unlink($upload['file']);
                    return new WP_Error('import_file_error', sprintf(esc_html__('Remote file is too large, limit is %s.', 'fona'), size_format($max_size)));
                }
            }
        } else {
